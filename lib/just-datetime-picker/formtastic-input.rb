    ...
    class JustDatetimePickerInput 
      ...
      def to_html
        input_wrapping do
          ...
          builder.number_field("#{method}_time_hour", input_html_options.merge({ :class => "just-datetime-picker-field just-datetime-picker-time just-datetime-picker-time-hour", :value => hour_value, :maxlength => 2, :size => 2, min: 0, max: 23 })) <<
          ":" <<
          builder.number_field("#{method}_time_minute", input_html_options.merge({ :class => "just-datetime-picker-field just-datetime-picker-time just-datetime-picker-time-minute", :value => minute_value, :maxlength => 2, :size => 2, min: 0, max: 59 }))
        end
      end
    end
  end
